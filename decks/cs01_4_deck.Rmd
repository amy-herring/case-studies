...
Variance Estimation:
$$ \mathbb{V}(\hat{\tau}) = \sum_{k=1}^K \{\mathbb{V}(\bar{Y}_{k,1}) - \mathbb{V}(\bar{Y}_{k,0})\}(\frac{N_{k,1}+N_{k,0}}{N})^2 $$
...
```{r, message=FALSE}
library(Matching)
match1 <- Match(Y=birth_data_overlap$BWTG, Tr = birth_data_overlap$smoke_LT, X = ps_overlap, version='fast', estimand="ATE")
summary(match1)
```

We then can check the resulting balance between treated and control across covariates with `MatchBalance`
```{r, eval=FALSE}
mb <- MatchBalance(smoke_LT ~ PARITY+GEST+MAGE+as.factor(MRACER)+ as.factor(MHISP), data= birth_data_overlap,match.out=match1)
```
 
 
## Exercises:
1. Estimate the ATE and variance using stratification.

  * Be sure to check balance after fitting propensity score model
2. How does that compare to the ATE from Matching?
3. What conclusions can we draw from 3rd trimester smoking?
4. Redo analysis with treatment defined as any trimester smoking.