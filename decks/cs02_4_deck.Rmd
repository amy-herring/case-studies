...
setwd("~/Desktop/Finished Document")
```
...
Also, note that the total infant mortality rate is a weighted average of the males and females (not the sum).
...
If we want some of the plots, e.g. the bar plots, to change depending on the country, we can filter by country. Click on the the map and in the upper right corner click on the drop down arrow and use the menu to filter by country. Then we want to "Use All" countries.
