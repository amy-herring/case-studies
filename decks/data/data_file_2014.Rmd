...
cia_ctr <- read.csv("countries.csv", header = TRUE)
cia_ctr <- cia_ctr[,1:2]

# population Data
popDF <- read.delim("factbook/rankorder/rawdata_2119.txt", header = FALSE, col.names = c("", "country","pop"))
popDF <- popDF[,-1] # remove blank column
popDF$pop <- as.numeric(gsub(",", "", popDF$pop))
popDF <- merge(cia_ctr, popDF, by.x="Entity", by.y = "country") #remove commas, so it can be converted to numeric
popDF <- popDF[,2:3]
names(popDF) <- c("ctry", "pop")
save(popDF, file="popDF.RData")

# Infant Mortality Data 
infMortDF <- read.delim("factbook/rankorder/rawdata_2091.txt", header = FALSE, col.names = c("", "country","infMort", ""))
infMortDF <- infMortDF[,-c(1,4)]
save(infMortDF, file="infMortDF.RData")
infMortDF <- merge(cia_ctr, infMortDF, by.x="Entity", by.y = "country")
