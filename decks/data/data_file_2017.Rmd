...
#iso_codes <- ctr_codes[,c(1,3)]

setwd("~/Desktop/factbook/rankorder")

popDF <- read_table("rawdata_2119.txt", col_names = c("X1", "country","X3", "X4", "pop"))
popDF <- popDF[,-c(1,3,4)] # remove blank columns
popDF <- merge(cia_codes, popDF, by.x="Country", by.y = "country") 
popDF <- popDF[,2:3]
names(popDF) <- c("ctry", "pop")
save(popDF, file="popDF.RData")

infMortDF <- read_table("rawdata_2091.txt", col_names = c("X1", "country","X3", "X4", "infMort"))
infMortDF <- infMortDF[,-c(1,3,4)]
...
IM <- matrix(0, 231, 4)
colnames(IM) <- c("country", "IM_overall_avg", "IM_male_avg", "IM_female_avg")
for(i in 1:231){
  IM[i, 1] <- t[[1]][i,1]
