...
These polls should include some respondent demographic information and some type of geographic indicator such as state. In this tutorial we'll be using the "gay_marriage_megapoll.dta" data, which contain results from five national polls that were conducted in 2004.
...
We may wish to model state-level effects in our multilevel regression. To do this we will use state-level predictiors, e.g. demographics. This will reduce the unexplained group level variation. In this tutorial, we already have a data set with state-level information in it.
...
marriage.data$p.mormon.full <-Statelevel$p.mormon[marriage.data$state.initnum]# proportion of LDS church members in respondent's state
marriage.data$p.relig.full <- marriage.data$p.evang.full + marriage.data$p.mormon.full# combined evangelical + LDS proportions
marriage.data$p.kerry.full <- Statelevel$kerry.04[marriage.data$state.initnum]# John Kerry's % of 2-party vote in respondent's state in 2004
