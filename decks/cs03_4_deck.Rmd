...
```{r model1, eval=FALSE}
library(lme4)
m1=glmer(bush ~ black + female + (1|state), family=binomial(link="logit"),
         data=polls.subset)
summary(m1)
```
...
library(lme4)
m1=glmer(bush ~ black + female + (1|state), family=binomial(link="logit"), data=polls.subset)
summary(m1)
```
</small>
...
polls.subset$v.prev.full=v.prev[polls.subset$state]*100 #rescaling so 1-unit is 1 % point
...
m2=glmer(bush ~ black + female + v.prev.full+ (1|state)+(1|age.edu)+(1|region.full), family=binomial(link="logit"), data=polls.subset)
summary(m2)
```
...
m3=brm(bush ~ black + female + v.prev.full+ (1|state)+(1|age.edu)+(1|region.full), family=bernoulli(link="logit"), control = list(adapt_delta = 0.995), data=polls.subset)

```

## Extracting sampled values

```{r brmssamp, cache=TRUE}
