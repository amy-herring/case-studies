...
```{r morepackages, message=FALSE,cache=TRUE}
library(tidyverse)
library(lme4)
library(brms)
library(rstan)
...
## Gather national opinion poll(s)
...
## Create a single internally-consistent dataset.
...
## Create dataset of state-level predictors
...
## Gather census data for poststratification
...
These averages will not be representative of the actual state mean if the sampled respondents are not in proportion to each group's percentage of the total state population. So we'll next poststratify.

## Poststratifying sample estimates

First, we find within-group averages.

```{r groupmeans}
grp.means=marriage.data %>% 
  group_by(state,region,race.female,age.edu.cat,p.relig.full,p.kerry.full) %>%
...
```{r grouppct}
grp.means=Census %>%
  select(cstate,cage.edu.cat,cregion,kerry.04,race.female,p.relig,cpercent.state) %>%
